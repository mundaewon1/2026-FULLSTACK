...
```bash
#1. 프로젝트별 독립된 파이썬 라이브러리 실행환경 (virtual environment)
# 작업 폴더 이동 후 가상환경 생성 
python -m venv venv
...
```bash
#  타언어의  MVC 패턴과 거의동일  (mvc=mtv)
#  JAVA
#  Model    - 데이터관련  : Entity/Dao/Dto
#  Template - 화면구성    : html, jsp, Thymeleaf,,,
#  View     - 비즈니스로직 : Controller,,,,
```
...
# 답: 1) ( 비즈니스 로직 처리 ) / 2) ( DB와 Template 연결 )
...
# Q1. 전체 웹사이트 설정 집합은 ( 프로젝트)이고, 독립적인 기능 단위는 ( 앱 )입니다.
# 답: (  )

# Q2. 로컬 개발 서버를 구동하는 명령어는 무엇인가요?
# 답: python manage.py ( runserver )
```
...
│   ├── __init__.py
│   ├── settings.py    <-- [설정] INSTALLED_APPS 및 타임존 설정
│   ├── urls.py        <-- [라우팅] 메인 URL 라우팅 및 리다이렉트 설정
│   └── wsgi.py
│
...
│   ├── apps.py
│   ├── models.py      <-- [DB] ServiceLog 데이터 모델 정의  ( Jpa Entity )
│   ├── views.py       <-- [로직] Pandas 데이터 집계 및 뷰 로직 ( Spring Controller 역할 )
...
# 2. DB 적용 - db.sqlite3데이터베이스에 반영해서 테이블을 생성
...
# 답: 1) python manage.py ( makemigrations ) / 2) python manage.py ( migrate )
...
## M(model) T(html,css:화면) V(Controller:비즈니스로직, db와 view연결)
