...
// 서버 BASE URL : 10.0.0.2 / localhost:8080 / https://thejoa703.duckdns.org
...
class BoardNotifier extends Notifier<BoardState> {
  ...
  void _initDio() {
    _dio = Dio(BaseOptions(  // 기본옵션 설정
      baseUrl: ApiClient.getBaseUrl(), // 10.0.0.2 / localhost:8080 / https://thejoa703.duckdns.org
    ));
  ...
  Future<bool> createPost({
    ...
    try {
      ...
      if (imageFiles != null && imageFiles.isNotEmpty) { // 첨부이미지가 있으면
...
flutter_launcher_icons:
  android: true
  ios: true
  image_path: "assets/images/app_logo.png"  ## 앱아이콘
  min_sdk_android: 21
...
```bash
flutter pub get
dart run flutter_launcher_icons  # 아이콘생성스크립트
