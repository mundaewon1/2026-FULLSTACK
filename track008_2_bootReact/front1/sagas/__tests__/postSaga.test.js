...
describe('auth saga', ()=>{
    ...
    it('fetchPostDetail success', () => {
        const generator = fetchPostDetail(fetchPostsDetailRequest(1));
    ...
    it('createPost success', () => {
        ...
        const generator = createPost(createPostsRequest(payload));
        ...
        expect(putStep).toEqual(put(createPostsSuccess(mockData)));
    });
 
    // --- 글수정 ---
    it('updatePost success', () => {
        const payload = { id: 10, content: 'updated' };
        const generator = updatePost(updatePostsRequest(payload));
        
        expect(generator.next().value.type).toBe('CALL');
        
        const putStep = generator.next({ data: payload }).value;
        
        expect(putStep).toEqual(put(updatePostsSuccess(payload)));
    });
 
    // --- 글삭제 ---
    it('deletePost success', () => {
        const generator = deletePost(deletePostsRequest(1));
        
        expect(generator.next().value.type).toBe('CALL');
        
        const putStep = generator.next().value;
        
        expect(putStep).toEqual(put(deletePostsSuccess(1)));
    });
