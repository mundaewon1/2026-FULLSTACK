...
class Receiver extends Thread{
	...
	public Receiver(Socket socket) { super(); 
		this.socket = socket; 
...
class Sender extends Thread{
	...
	public Sender() { super(); }
	public Sender(Socket socket) { super(); 
...
2. Socket 통신 -  양방향 (특정포트를 통해 실시간으로 정보주고받음 - tcp/udp)
3. 소켓통신흐름
   1) 서버소켓( as 센터) , 포트바인딩( 문열기)
   2) 클라이언트 연결 요청, 수락
   3) 클라이언 소켓(socket) ↔ 상담사(socket) ( InputStream  > 프로그램기준 > OutputStream )
