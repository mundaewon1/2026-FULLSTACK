...
-- Q1. 나이가 많은 순으로 출력
select * from select_userinfo order by age desc;  -- 내림차순  where 생략가능
...
-- Q2. 나이가 많은 순으로 2명 출력
 ...
 select no,age from select_userinfo order by age desc limit 1,5; -- 어디서부터 , 몇개 (최신부터, 2개)
...
select ename,sal,empno from emp where sal >= 2000 order by empno desc;
...
-- 14 rows in set (0.00 sec)
--  * 컬럼명에 별칭주기(as) 
select empno `사원번호`, ename`이름`, job`담당업무` from emp;
...
-- 실습 11 emp 테이블에서 급여를 기준으로 내림차순으로 정렬하여 조회하시오.
-- +-------+--------+-----------+------+------------+------+------+--------+
-- | empno | ename  | job       | mgr  | hiredate   | sal  | comm | deptno |
-- +-------+--------+-----------+------+------------+------+------+--------+
-- |  7839 | KING   | PRESIDENT | NULL | 1981-11-17 | 5000 | NULL |     10 |
...
-- +-------------+---------------+-----------+---------+------------+--------+------------+---------------+
-- 14 rows in set (0.00 sec)
select empno`EMPLOYEE_NO`,ename`EMPLOYEE_NAME`,job`JOB`,mgr`MANAGER`,hiredate`HIREDATE`,sal`SALARY`,comm`COMMISSION`,deptno`DEPARTMENT_NO` 
from emp
