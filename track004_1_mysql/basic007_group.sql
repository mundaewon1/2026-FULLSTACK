...
select ban, count(*) -- 0 (집계안된 일반), 1(집계)
from group_userinfo;

select		ban, count(*)	-- 반, 갯수
from		group_userinfo
-- where
group by	ban;			-- 반별 ( 집계필드 빼고 다)
-- having
-- order by
-- limit 
...
-- +------+----------+----------+----------+
-- 3 rows in set (0.00 sec)
select		ban, sum(kor) `국어총합`, sum(eng), sum(math)
from		group_userinfo
group by 	ban;
-- having
...
-- +----------+
-- 1 row in set (0.00 sec)
select sum(comm)`추가수당` from emp;  -- sum할 시 null값 빼고 계산해줌 - 집계함수
...
-- +------------+
-- 1 row in set (0.00 sec)
select count(*)`데이터갯수`, count(mgr)`null빼고 계산` from emp; -- 집계함수는 null 빼고 다
...
-- +---------------+
-- 1 row in set (0.00 sec)
select count(*)`부서30 직원수` from emp where deptno = 30;		-- 집계전 필터링
select count(*)`부서30 직원수` from emp group by deptno having deptno = 30;  -- 집계후 필터링
...
-- +-------------------+
-- 1 row in set (0.00 sec)
select hiredate`부서20 최근입사일` from emp where deptno = 20 order by hiredate desc limit 1;
...
-- +---------------------------+
-- 1 row in set (0.00 sec)
select hiredate`부서20 제일오래 된 입사일` from emp where deptno = 20 order by hiredate asc limit 1;
...
-- +--------+-----------+
-- 3 rows in set (0.00 sec)
select deptno, avg(sal)`평균급여` from emp group by deptno order by deptno desc;
