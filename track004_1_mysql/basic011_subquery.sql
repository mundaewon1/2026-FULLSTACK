...
select b.deptno, dname, avg(sal)`AVG_SAL`, max(sal)`MAX_SAL`, min(sal)`MIN_SAL`, count(*)`CNT`
from emp a, dept b
...
select b.deptno, dname, empno, ename, job, sal
...
-- +-------+-------+---------+------+--------+----------+--------+
-- 3 rows in set (0.00 sec)
select empno, ename, job, sal, a.deptno, dname, loc
from emp a, dept b
where a.deptno = b.deptno and a.deptno = 20 and sal > (select avg(sal) from emp )
order by empno desc;
...
select *
from emp
where sal in ( select  max(sal) `부서최고급여`
						from emp
						group by deptno);
