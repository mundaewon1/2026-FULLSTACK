...
update join_userinfo set age=11 where no=1;
...
select a.no, a.name, a.age, b.ban
from join_userinfo a join join_userban b
on a.no = b.no and age between 20 and 40 ;
...
from join_userinfo a
right join join_userban b
on a.no = b.no;

select b.no, b.name, a.ban, b.age
from join_userban a
left join join_userinfo b
on a.no = b.no;
...
-- -- 이코드가 의미하는바는? emp, dept 14줄*4줄 테이블들의 데이터 empno 기준 오름차순으로
-- -- 카다시안곱 - 모든조합 - 조인조건이 없으면 두테이블 모두 조회
...
-- SELECT EMPNO, ENAME, DEPTNO, DNAME, LOC  소속이 없음 
-- FROM EMP E, DEPT D
...
from emp a, emp b ,salgrade s
where a.mgr = b.empno and a.sal between s.losal and s.hisal
order by b.empno;
...
order by deptno;  -- deptno 필드 겹침 부분이 1번만나옴 
...
select  empno, ename, job, mgr, hiredate, sal, comm, a.deptno, dname, loc
from emp a join dept b
on a.deptno = b.deptno and  sal <=3000
order by a.deptno;
...
-- +--------+------------+-----------+---------+---------+-----+
-- 3 rows in set (0.00 sec)
select b.deptno, b.dname, avg(a.sal)`AVG_SAL`, max(a.sal)`MAX_SAL`, min(a.sal)`MIN_SAL`, count(a.sal)`CNT`
from emp a, dept b
where a.deptno = b.deptno
...
-- +--------+------------+-----------+---------+---------+-----+
-- 3 rows in set (0.00 sec)
select deptno, dname, avg(sal)`AVG_SAL`, max(sal)`MAX_SAL`, min(sal)`MIN_SAL`, count(sal)`CNT`
from emp join dept
using(deptno)
...
-- +--------+------------+-------+--------+------+------+--------+-------+-------+-------+-----------+-----------+
-- 15 rows in set (0.00 sec)
select a.deptno, a.dname, sawon.empno, sawon.ename, sawon.mgr, sawon.sal, sawon.deptno, s.losal, s.hisal, s.grade, mgr.empno`MGR_EMPNO`, mgr.ename`MGR_ENAME`
from dept a 
left join emp sawon  on (sawon.deptno=a.deptno) 
left join salgrade s on (sawon.sal between s.losal and s.hisal) 
left join emp mgr	 on (sawon.mgr = mgr.empno)  
order by a.deptno, sawon.ename;
