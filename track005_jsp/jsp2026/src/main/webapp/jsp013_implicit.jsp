...
<body>
   ...
   <div class="container card my-5">
	  ...
	  검색어 : 입력한값
   	  ...
   	  <form  action="jsp013_1.jsp"" method="get"  onsubmit="return ck()">
	      <div class="my-2">
	      	<label for="query" >검색어</label>
	      	<input type="text" class="form-control" placeholder="입력하세요"  id="query" name="query"	/>
	      	<button type="submit" class="btn btn-success my-3 d-block">검색</button>
	      </div>
      </form>
      ...
      <form action="jsp013_4.jsp"  method="get"  onsubmit="return ck2()"  >
        <div class="my-2">
          <label for="userage">나이입력</label> 
          <input  type="text" class="form-control" placeholder=""  id="userage"name="userage">
          <div class="my-2">
          <button  type="submit"  class="btn btn-primary" >성인여부</button>
          </div>
         </div>                  
      </form>
      ...
      <%
	      application.setAttribute("name", "D. application-웹애플리케이션 실행");
	     	  session.setAttribute("name", "C.   session -브라우저종료 / 로그인 후 아이디 유지");
	      	  request.setAttribute("name", "B.	 request -a.jps -> b.jsp 요청할때/현재페이지에 요청");
